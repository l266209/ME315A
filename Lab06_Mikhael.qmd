...
```{r}
dbGetQuery(conn, "SELECT COUNT(*) AS CLIENTES FROM customers")
```

6.  Utilizando apenas SQLite, identifique o número de países diferentes em que moram os clientes encontrados acima.

```{r}
dbGetQuery(conn, "SELECT COUNT(DISTINCT Country) AS PAÍSES FROM customers")
```
...
```{r}
dbGetQuery(conn, "
  SELECT Country AS PAÍSES, COUNT(*) AS CLIENTES
  FROM customers
  GROUP BY PAÍSES
