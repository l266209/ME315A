---
title: "Lista03_Mikhael"
author: "Leonardo - 266209"
format: pdf
editor: visual
---
...
SELECT DISTINCT DNO, SALARY FROM EMPLOYEE ORDER BY DNO, SALARY
```
  A 1a e a 2a QUERYS são idênticas, pois não há observações de DNO e SALARY (juntos) que sejam iguais.
  A 3a QUERY apenas ordena essas observações por DNO e SALARY.
...
15. As seguintes cláusulas 'WHERE' são logicamente equivalentes?
  ...
  Sim, pois C1 AND (C2 OR C3) = (C1 AND C2) OR (C1 AND C3).
...
16. As seguintes cláusulas 'WHERE' são logicamente equivalentes?
  ...
  Sim, pois C1 OR (C2 AND C3) = (C1 OR C2) AND (C1 OR C3).
...
17. As seguintes cláusulas 'WHERE' são logicamente equivalentes?
  ...
  A 1a e a 2a cláusulas não são logicamente equivalentes, pois NOT (C1 AND C2) = (NOT C1) OR (NOT C2).
  A 1a e a 3a cláusulas são logicamente equivalentes, pois NOT (C1 AND C2) = (NOT C1) OR (NOT C2).
...
18. As seguintes cláusulas 'WHERE' são logicamente equivalentes?
  ...
  A 1a e a 2a cláusulas não são logicamente equivalentes, pois NOT (C1 OR C2) = (NOT C1) AND (NOT C2).
  A 1a e a 3a cláusulas são logicamente equivalentes, pois NOT (C1 OR C2) = (NOT C1) AND (NOT C2).
